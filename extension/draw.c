...
// Draw message for user to drag and drop assembly file, only beginning
void draw_drop_file_message(void) {
    SDL_Surface *drop_file_text = TTF_RenderText_Blended(
        arial_24, 
        "Please drag and drop assembly file here.", 
        BLACK
    );
    SDL_Texture *drop_file_texture = SDL_CreateTextureFromSurface(
        renderer, 
        drop_file_text
    );
    SDL_Rect drop_file_rect = {
        .x = 25, 
        .y = 620, 
        .w = drop_file_text->w, 
        .h = drop_file_text->h
    };
    SDL_RenderCopy(renderer, drop_file_texture, NULL, &drop_file_rect);

    SDL_FreeSurface(drop_file_text);
    SDL_DestroyTexture(drop_file_texture);
}

// Draw message for user that assembly file was received 
// and successfully assembled
void draw_dropped_file_message(void) {
    SDL_Surface *drop_file_text = TTF_RenderText_Blended(
        arial_24, 
        "Received assembly file and successfully assembled, press Step to begin emulation.", 
        BLACK
    );
    SDL_Texture *drop_file_texture = SDL_CreateTextureFromSurface(
        renderer, 
...
// Draw the current instrution being executed for indication
void draw_instruction(char *instruction) {
    if (instruction == NULL) {
        return;
    }
    SDL_Surface *instruction_text = TTF_RenderText_Blended(
        arial_24, 
        instruction, 
        BLACK
    );
    SDL_Texture *instruction_texture = SDL_CreateTextureFromSurface(
        renderer, 
        instruction_text
    );
    SDL_Rect instruction_rect = {
        .x = 25, 
        .y = 620, 
        .w = instruction_text->w, 
        .h = instruction_text->h
    };
    SDL_RenderCopy(renderer, instruction_texture, NULL, &instruction_rect);

    SDL_FreeSurface(instruction_text);
    SDL_DestroyTexture(instruction_texture);
}

// Draw message that emulator has halted
void draw_halted(void) {
    SDL_Surface *halted_text = TTF_RenderText_Blended(
        arial_24, 
        "Halted. Please Drag and drop assembly file to run again.", 
        BLACK
    );
    SDL_Texture *halted_texture = SDL_CreateTextureFromSurface(
        renderer, 
        halted_text
    );
    SDL_Rect halted_rect = {
        .x = 25, 
        .y = 620, 
        .w = halted_text->w, 
        .h = halted_text->h
    };
    SDL_RenderCopy(renderer, halted_texture, NULL, &halted_rect);

    SDL_FreeSurface(halted_text);
    SDL_DestroyTexture(halted_texture);
}
...
void draw_button(button_t *button) {
    ...
    SDL_Rect text_rect = {
        .x = button->rect.x + button->rect.w / 2 - text_surface->w / 2,
        .y = button->rect.y + button->rect.h / 2 - text_surface->h / 2,
        .w = text_surface->w,
        .h = text_surface->h
    };
...
void draw_main_components(circuit_state_t state) {
